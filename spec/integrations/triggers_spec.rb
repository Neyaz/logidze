...
describe "Logidze triggers with except", :db do
  include_context "cleanup migrations"
  ...
  before(:all) do
    ...
    Dir.chdir("#{File.dirname(__FILE__)}/../dummy") do
      successfully "rails generate logidze:install"
      successfully "rails generate logidze:model post --limit=4 --except='created_at,rating' --backfill"
      successfully "rake db:migrate"
...
describe "Logidze triggers with only", :db do
  include_context "cleanup migrations"
  ...
  before(:all) do
    ...
    Dir.chdir("#{File.dirname(__FILE__)}/../dummy") do
      successfully "rails generate logidze:install"
      successfully "rails generate logidze:model post --limit=4 --only='created_at,rating' --backfill"
      successfully "rake db:migrate"
  ...
  describe "undo/redo" do
    ...
    it "removes future version when updated after undo" do
      ...
      expect(post.log_size).to eq 2
      expect(post.rating).to eq 10
      expect(post.title).to eq "No Future"
    end
